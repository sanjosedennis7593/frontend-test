...
const UserRepositories = props => {
    ...
    const { data } = useApiQuery(REPOSITORY_BY_USER_QUERY, {
                ...
                <div class="flex p-3 border-l-8 border-yellow-600">
                    <div class="flex-1">
                        <div class="ml-3 space-y-1 pr-3">
                            <div class="text-base leading-6 font-normal"><Link to={`/repository?repo_id=${item.id}&repo_name=${item.name}&owner=${user.node.login}`}>{item.name}</Link></div>
